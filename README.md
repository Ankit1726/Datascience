# ***AI/ML Learning Vault***

<div align="center">
  
🧠 16 GitHub Repositories Every AI/ML Engineer Should Know

Your Complete Roadmap to Machine Learning, Deep Learning, LLMs, RAG, NLP & Agentic AI

A curated collection of the most valuable open-source repositories for aspiring AI/ML Engineers, Data Scientists, and Generative AI Developers.

![AI](https://img.shields.io/badge/Artificial%20Intelligence-AI-blue)
![ML](https://img.shields.io/badge/Machine%20Learning-ML-green)
![LLM](https://img.shields.io/badge/Generative%20AI-LLMs-orange)

</div>

---

## 📚 Machine Learning Foundations

### 1. [Machine Learning for Beginners (Microsoft)](https://github.com/microsoft/ML-For-Beginners)

- Complete Machine Learning curriculum
- Hands-on projects and exercises
- Beginner-friendly learning path

### 2. [100 Days of ML Coding](https://github.com/Avik-Jain/100-Days-Of-ML-Code)

- Daily Machine Learning roadmap
- Practical coding exercises
- Build consistency and problem-solving skills

### 3. [Algorithms Implemented in Python](https://github.com/TheAlgorithms/Python)

- 200+ algorithms in Python
- Strong programming and DSA foundations
- Well-documented implementations

### 4. [Mathematics for Machine Learning](https://github.com/mml-book/mml-book.github.io)

- Linear Algebra
- Calculus
- Probability & Statistics
- Essential mathematics for ML

### 5. [Made With ML](https://github.com/GokuMohandas/Made-With-ML)

- End-to-end ML workflows
- Data collection to deployment
- Production-ready ML practices

---

## 🤖 Deep Learning

### 6. [Deep Learning Paper Implementations](https://github.com/labmlai/annotated_deep_learning_paper_implementations)

- Research paper reproductions
- Learn modern Deep Learning architectures
- Code-first learning approach

### 7. [Neural Networks: Zero To Hero](https://github.com/karpathy/nn-zero-to-hero)

- Build Neural Networks from scratch
- Understand backpropagation
- Learn Deep Learning fundamentals

---

## 🦾 Large Language Models & Agentic AI

### 8. [Hands-On Large Language Models](https://github.com/HandsOnLLM/Hands-On-Large-Language-Models)

- Practical LLM applications
- Embeddings, Fine-tuning & RAG
- Modern Generative AI concepts

### 9. [Prompt Engineering Guide](https://github.com/dair-ai/Prompt-Engineering-Guide)

- Prompting frameworks
- Advanced prompting techniques
- Real-world examples

### 10. [AI Agents for Beginners](https://github.com/microsoft/ai-agents-for-beginners)

- Agentic AI fundamentals
- Multi-Agent systems
- Hands-on AI Agent projects

### 11. [Generative AI Agent Techniques](https://github.com/NirDiamant/GenAI_Agents)

- AI Agent architectures
- Autonomous workflows
- Advanced Agent design patterns

### 12. [RAG Techniques Repository](https://github.com/NirDiamant/RAG_Techniques)

- 25+ RAG techniques
- Retrieval optimization methods
- Production-ready implementations

---

## 📊 Data Science & NLP

### 13. [Real-World Data Science Projects](https://github.com/krishnaik06/Complete-Data-Science-With-Machine-Learning-And-NLP-2025)

- Industry-level projects
- End-to-end ML pipelines
- Portfolio-ready applications

### 14. [Awesome Natural Language Processing](https://github.com/keon/awesome-nlp)

- NLP tools and libraries
- Datasets and research papers
- Comprehensive NLP resources

---

## 🎮 Reinforcement Learning

### 15. [Awesome Reinforcement Learning](https://github.com/aikorea/awesome-rl)

- Curated RL resources
- Tutorials and learning materials
- Popular RL libraries

### 16. [Reinforcement Learning Algorithms From Scratch](https://github.com/dennybritz/reinforcement-learning)

- Learn RL step-by-step
- Implement algorithms yourself
- Understand RL foundations deeply

---

## 🎯 Recommended Learning Path

```text
Python
   ↓
Mathematics
   ↓
Machine Learning
   ↓
Deep Learning
   ↓
NLP & Computer Vision
   ↓
Large Language Models
   ↓
Prompt Engineering
   ↓
RAG Systems
   ↓
AI Agents
   ↓
Production AI Applications
```

---

## 📌 Topics Covered

- Machine Learning
- Deep Learning
- Neural Networks
- Data Science
- Natural Language Processing (NLP)
- Large Language Models (LLMs)
- Prompt Engineering
- Generative AI
- Agentic AI
- RAG (Retrieval-Augmented Generation)
- Reinforcement Learning
- MLOps

---

## 🌟 Why This Collection?

✅ Free & Open Source  
✅ Beginner Friendly  
✅ Industry Relevant  
✅ Production-Oriented Learning  
✅ Real-World Projects  
✅ Portfolio Building  
✅ Interview Preparation  
✅ Curated Learning Path  

---

### **👨‍💻 Author: Ankit Gupta**

**AI/ML Engineer | Generative AI & Agentic AI Developer**

- 🚀 Building AI Applications  
- 📚 Learning in Public  
- 🤝 Sharing Knowledge with the Community  

---

## ⭐ Support This Repository

>If you found this repository useful:

- Star ⭐ the repository
- Fork 🍴 the repository
- Share 📢 with fellow learners
- Follow for more AI/ML content

---

<div align="center">

#### ***Learn • Build • Share • Grow***

</div
